...
import PutBackIcon from '../icon/put-back-icon';
...
const FileFeedback = ({
  ...
  return (
    <div className="text-sm text-text">
      <div className="flex justify-end gap-2">
        <div
          className={`flex w-max items-center gap-2 cursor-pointer border transition border-border rounded py-[0px] px-2 hover:text-text  hover:bg-background ${
            feedbackOpen ? 'bg-backgroud' : 'bg-backgroud_second'
          }`}
        ...
        <div className="self-end cursor-pointer p-1 h-6 w-6">
          <button onClick={() => handleSourceClick(source)}>
            {source.relevant ? <TrashIcon /> : <PutBackIcon />}
          </button>
        </div>
      </div>
